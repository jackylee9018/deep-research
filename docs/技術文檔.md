...
路由 **`/meeting`**：上傳 MP3/WAV → 本地 **WhisperX worker** 轉錄 + 說話者分段 → **vLLM / OpenRouter** 產出結構化會議紀要（Markdown，可匯出 Word/PDF）。

**Mac（Apple Silicon）啟動：**

```bash
# 1. Hugging Face token（pyannote 說話者分段）
export HF_TOKEN="hf_..."
...
環境變數見 `.env.example`：`WHISPERX_WORKER_URL`、`WHISPERX_MODEL`（Mac 建議 `medium`）、`WHISPERX_DEVICE=cpu`。

Worker 原始碼：`services/whisperx-worker/`。API：`POST /api/meeting/process`（multipart 音訊 + SSE 進度）。
